...
./reminder.sh complete --title "去领快递" --match-time "15:00"
./reminder.sh complete --title "去领快递" --match-date "2026-02-11" --match-time "15:00"
```
...
# 🆕 多维度消歧：通过标题+时间精准删除
./reminder.sh delete --title "去领快递" --match-time "15:00"
./reminder.sh delete --title "去领快递" --match-date "2026-02-11" --match-time "15:00"
```
...
- **完成任务**: `--match-date` / `--match-time`
- **更新任务**: `--match-date` / `--match-time` (用于定位) + `--date` / `--time` (用于修改)
- **删除任务**: `--match-date` / `--match-time`

**示例**：
```bash
# 场景：有两个"去领快递"，分别在 15:00 和 16:00
./reminder.sh complete --title "去领快递" --match-time "15:00"  # 只完成 15:00 那个
```
