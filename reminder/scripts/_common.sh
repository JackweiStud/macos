...
reminders_to_json() {
    local action="$1"
    jq -R 'split("\t") | {id:.[0],name:.[1],due_date:(.[2] | select(.!="") // null),list:.[3],notes:(.[4]//""),completed:(.[5]=="true")}' \
    | jq -s --arg a "$action" \
...
reminder_to_json() {
    ...
    echo "$line" | jq -R 'split("\t") | {id:.[0],name:.[1],due_date:(.[2] | select(.!="") // null),list:.[3],notes:(.[4]//""),completed:(.[5]=="true")}' \
